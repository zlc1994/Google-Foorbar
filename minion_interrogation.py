...
def answer(minions):
    """Dividing time by the probability of the success will get each
    expected time. Sort the minions by the increasing expected time.
    """
    expect = [time/(num/den) for time, num, den in minions]

    return sorted(range(len(expect)), key=lambda x:expect[x])
